...
local int
add_data_in_datablock (linkedlist_data * ll, const void *buf, uLong len)
{
  linkedlist_datablock_internal *ldi;
...
local int
zip64local_putValue (const zlib_filefunc64_32_def * pzlib_filefunc_def,
...
local void
zip64local_putValue_inmemory (void *dest, ZPOS64_T x, int nbByte)
{
...
local uLong
zip64local_TmzDateToDosDate (const tm_zip * ptm)
{
...
local int
zip64local_getByte (const zlib_filefunc64_32_def * pzlib_filefunc_def,
...
local int
zip64local_getShort (const zlib_filefunc64_32_def * pzlib_filefunc_def,
...
local int
zip64local_getLong (const zlib_filefunc64_32_def * pzlib_filefunc_def,
		    voidpf filestream, uLong * pX)
{
  uLong x;
  int i = 0;
...
local int
zip64local_getLong64 (const zlib_filefunc64_32_def * pzlib_filefunc_def,
			       ...
			       pzlib_filefunc_def, voidpf filestream)
{
  ...
  if (ZSEEK64
      (*pzlib_filefunc_def, filestream, uPosFound,
       ZLIB_FILEFUNC_SEEK_SET) != 0)
    return 0;
  ...
  if (ZSEEK64
      (*pzlib_filefunc_def, filestream, relativeOffset,
       ZLIB_FILEFUNC_SEEK_SET) != 0)
    return 0;
...
int
LoadCentralDirectoryRecord (zip64_internal * pziinit)
{
  int err = ZIP_OK;
  ...
  if (central_pos > 0)
    ...
    {
      central_pos =
	zip64local_SearchCentralDir (&pziinit->z_filefunc,
				     pziinit->filestream);
    }
...
*/
  ...
  if (hasZIP64Record)
    {
      ...
      if (ZSEEK64
	  (pziinit->z_filefunc, pziinit->filestream, central_pos,
	   ZLIB_FILEFUNC_SEEK_SET) != 0)
	err = ZIP_ERRNO;
	...
	err = ZIP_ERRNO;
      ...
      if (zip64local_getLong64
	  (&pziinit->z_filefunc, pziinit->filestream,
	...
	err = ZIP_ERRNO;
      ...
      /* number of this disk */
      if (zip64local_getLong
	  (&pziinit->z_filefunc, pziinit->filestream, &number_disk) != ZIP_OK)
	err = ZIP_ERRNO;
  ...
  if (size_comment > 0)
	...
	{
	  size_comment =
	    ZREAD64 (pziinit->z_filefunc, pziinit->filestream,
...

  {
	...
	if (ZREAD64
	    (pziinit->z_filefunc, pziinit->filestream, buf_read,
	     (uLong) read_this) != read_this)
	  err = ZIP_ERRNO;

	if (err == ZIP_OK)
	  err =
	    add_data_in_datablock (&pziinit->central_dir, buf_read,
				   (uLong) read_this);
...
extern zipFile ZEXPORT
zipOpen2 (const char *pathname, int append, zipcharpc * globalcomment,
...
extern zipFile ZEXPORT
zipOpen2_64 (const void *pathname, int append, zipcharpc * globalcomment,
...
int
Write_LocalFileHeader (zip64_internal * zi, const char *filename,
 ...
 */
extern int ZEXPORT
zipOpenNewFileInZip4_64 (zipFile file, const char *filename,
			 ...
			 uLong flagBase, int zip64)
{
  ...
  zi->ci.central_header =
    (char *) ALLOC ((uInt) zi->ci.size_centralheader +
  ...
  err =
    Write_LocalFileHeader (zi, filename, size_extrafield_local,
			   extrafield_local);
  ...
  if ((err == ZIP_OK) && (zi->ci.method == Z_DEFLATED) && (!zi->ci.raw))
      ...
      if (zi->ci.method == Z_DEFLATED)
	{
	  ...
	  err =
	    deflateInit2 (&zi->ci.stream, level, Z_DEFLATED, windowBits,
			  memLevel, strategy);
		      ...
		      uLong flagBase)
{
  return zipOpenNewFileInZip4_64 (file, filename, zipfi,
				  ...
				  password, crcForCrypting, versionMadeBy,
				  flagBase, 0);
}
		      ...
		      int method, int level, int raw)
{
  return zipOpenNewFileInZip4_64 (file, filename, zipfi,
				  ...
				  Z_DEFAULT_STRATEGY, NULL, 0, VERSIONMADEBY,
				  0, 0);
}
...
zip64FlushWriteBuffer (zip64_internal * zi)
{
  ...
  if (zi->ci.encrypt != 0)
	...
	zi->ci.buffered_data[i] =
	  zencode (zi->ci.keys, zi->ci.pcrc_32_tab, zi->ci.buffered_data[i],
		   t);
#endif
    }
...
zipCloseFileInZipRaw64 (zipFile file, ZPOS64_T uncompressed_size, uLong crc32)
{
  ...
  if (err == ZIP_OK)
    err =
      add_data_in_datablock (&zi->central_dir, zi->ci.central_header,
  ...
  if (err == ZIP_OK)
    {
      ...
      if (ZSEEK64
	  (zi->z_filefunc, zi->filestream, zi->ci.pos_local_header + 14,
	   ZLIB_FILEFUNC_SEEK_SET) != 0)
	err = ZIP_ERRNO;
      ...
      else
	{
      ...
      if (ZSEEK64
	  (zi->z_filefunc, zi->filestream, cur_pos_inzip,
	   ZLIB_FILEFUNC_SEEK_SET) != 0)
	err = ZIP_ERRNO;
    }
...
int
Write_Zip64EndOfCentralDirectoryRecord (zip64_internal * zi,
					...
					ZPOS64_T centraldir_pos_inzip)
{
  ...
  if (err == ZIP_OK)		/* total number of entries in the central dir on this disk */
    err =
      zip64local_putValue (&zi->z_filefunc, zi->filestream, zi->number_entry,
			   8);
  ...
  if (err == ZIP_OK)		/* offset of start of central directory with respect to the starting disk number */
	...
	centraldir_pos_inzip - zi->add_position_when_writting_offset;
      err =
	zip64local_putValue (&zi->z_filefunc, zi->filestream, (ZPOS64_T) pos,
			     8);
    }
  return err;
}
...
int
Write_EndOfCentralDirectoryRecord (zip64_internal * zi, uLong size_centraldir,
				   ZPOS64_T centraldir_pos_inzip)
{
  int err = ZIP_OK;
...
int
Write_GlobalComment (zip64_internal * zi, const char *global_comment)
{
  int err = ZIP_OK;
  ...
  if (err == ZIP_OK && size_global_comment > 0)
    {
      if (ZWRITE64
	  (zi->z_filefunc, zi->filestream, global_comment,
...
zipClose (zipFile file, const char *global_comment)
{
  ...
  if (err == ZIP_OK)
      ...
      while (ldi != NULL)
	{
	  if ((err == ZIP_OK) && (ldi->filled_in_this_block > 0))
	    {
	      if (ZWRITE64
		  (zi->z_filefunc, zi->filestream, ldi->data,
  ...
  if (err == ZIP_OK)
    err =
      Write_EndOfCentralDirectoryRecord (zi, size_centraldir,
					 centraldir_pos_inzip);
