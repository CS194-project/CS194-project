...
voidpf
call_zopen64 (const zlib_filefunc64_32_def * pfilefunc, const void *filename,
	      int mode)
{
...
long
call_zseek64 (const zlib_filefunc64_32_def * pfilefunc, voidpf filestream,
	      ZPOS64_T offset, int origin)
{
  if (pfilefunc->zfile_func64.zseek64_file != NULL)
    return (*(pfilefunc->zfile_func64.zseek64_file)) (pfilefunc->zfile_func64.
						      opaque, filestream,
						      offset, origin);
  else
    {
...
void
fill_zlib_filefunc64_32_def_from_filefunc32 (zlib_filefunc64_32_def *
					     p_filefunc64_32,
...
ftell_file_func (voidpf opaque, voidpf stream)
{
  long ret;
...
ftell64_file_func (voidpf opaque, voidpf stream)
{
  ZPOS64_T ret;
...
static long ZCALLBACK
fseek_file_func (voidpf opaque, voidpf stream, uLong offset, int origin)
{
  int fseek_origin = 0;
  long ret;
  switch (origin)
...
static long ZCALLBACK
fseek64_file_func (voidpf opaque, voidpf stream, ZPOS64_T offset, int origin)
{
  int fseek_origin = 0;
  long ret;
  switch (origin)
...
static int ZCALLBACK
ferror_file_func (voidpf opaque, voidpf stream)
{
  int ret;
