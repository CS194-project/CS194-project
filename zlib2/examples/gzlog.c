...
local int
log_mark (struct log *log, int op)
{
  int ret;
...
local int
log_last (struct log *log, int last)
{
  int back, len, mask;
  unsigned char buf[6];
...
/* Append len bytes from data to the locked and open log file.  len may be zero
   ...
   the foo.gz file. */
local int
log_append (struct log *log, unsigned char *data, size_t len)
{
  uint put;
  off_t end;
  unsigned char buf[8];
...
/* Compress the len bytes at data and append the compressed data to the
   ...
   allocation failure. */
local int
log_compress (struct log *log, unsigned char *data, size_t len)
{
  int fd;
  uint got, max;
  ssize_t dict;
...
local void
log_log (struct log *log, int op, char *record)
{
  time_t now;
  FILE *rec;
...
/* Recover the interrupted operation op.  First read foo.add for recovering an
   ...
   allocation failure. */
local int
log_recover (struct log *log, int op)
{
  int fd, ret = 0;
...
/* gzlog_write() return values:
   ...
   -3: invalid log pointer argument */
int
gzlog_write (gzlog * logd, void *data, size_t len)
{
  int fd, ret;
  struct log *log = logd;
