...
local gzFile
gz_open (const void *path, int fd, const char *mode)
{
  gz_statep state;
  size_t len;
  int oflag;
...
/* -- see zlib.h -- */
gzFile ZEXPORT
gzopen64 (const char *path, const char *mode)
{
  return gz_open (path, -1, mode);
}
...
/* -- see zlib.h -- */
z_off64_t ZEXPORT
gzseek64 (gzFile file, z_off64_t offset, int whence)
{
  unsigned n;
  z_off64_t ret;
  gz_statep state;
...
/* Create an error message in allocated memory and set state->err and
   ...
   out of memory. */
void ZLIB_INTERNAL
gz_error (gz_statep state, int err, const char *msg)
{
  ...
  if ((state->msg =
       (char *) malloc (strlen (state->path) + strlen (msg) + 3)) == NULL)
    {
