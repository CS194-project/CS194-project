...
local int
unz64local_getByte (const zlib_filefunc64_32_def * pzlib_filefunc_def,
...
local int
unz64local_getShort (const zlib_filefunc64_32_def * pzlib_filefunc_def,
...
local int
unz64local_getLong (const zlib_filefunc64_32_def * pzlib_filefunc_def,
		    voidpf filestream, uLong * pX)
{
  uLong x;
  int i = 0;
...
local int
unz64local_getLong64 (const zlib_filefunc64_32_def * pzlib_filefunc_def,
		 ...
		 int is64bitOpenFunction)
{
  ...
  if (central_pos)
	...
	err = UNZ_ERRNO;
      ...
      if (unz64local_getLong64
	  (&us.z_filefunc, us.filestream, &number_entry_CD) != UNZ_OK)
	err = UNZ_ERRNO;
...
*/
local void
unz64local_DosDateToTmuDate (ZPOS64_T ulDosDate, tm_unz * ptm)
{
  ZPOS64_T uDate;
...
local int
unz64local_GetCurrentFileInfoInternal (unzFile file,
  ...
  if ((err == UNZ_OK) && (file_info.size_file_extra != 0))
      ...
      while (acc < file_info.size_file_extra)
	{
	  ...
	  if (headerId == 0x0001)
	    ...
	    {
	      if (ZSEEK64
		  (s->z_filefunc, s->filestream, dataSize,
		   ZLIB_FILEFUNC_SEEK_CUR) != 0)
		err = UNZ_ERRNO;
	    }
...
unzGetFilePos (unzFile file, unz_file_pos * file_pos)
{
  ...
  if (err == UNZ_OK)
	...
	(uLong) file_pos64.pos_in_zip_directory;
...
*/
extern int ZEXPORT
unzOpenCurrentFile3 (unzFile file, int *method,
