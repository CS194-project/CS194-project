...
*   Author  : Adnan Ozsoy, Martin Swany, Arun Chauhan,Indiana University - Bloomington
...
int compression_kernel_wrapper(unsigned char *buffer, int buf_length, unsigned char * bufferout, int compression_type,int wsize, int numthre, int noop,int index,unsigned char * in_d,unsigned char * out_d)
{
	int numThreads = numthre;
	//int instreams = 16; 
	int numblocks = buf_length / (PCKTSIZE*instreams);
	int i=0;
	...
	for(i = 0; i < instreams; i++)
	{
		...
		cudaMemcpyAsync(in_d+ i * (buf_length / instreams), buffer+ i * (buf_length / instreams), sizeof(char)*(buf_length / instreams),cudaMemcpyHostToDevice, streams[index*instreams + i]);
		checkCUDAError("mem copy to gpu");
	}
	
    for(i = 0; i < instreams; i++)
	{
		EncodeKernel<<< numblocks, numThreads, 0, streams[index*instreams + i]>>>(in_d + i * (buf_length / instreams),out_d + 2 * i * (buf_length / instreams),numThreads);
		checkCUDAError("kernel invocation");   // Check for any CUDA errors
	}
	for(i = 0; i < instreams; i++)
	{	
		cudaMemcpyAsync(bufferout + 2 * i * (buf_length / instreams), out_d + 2 * i * (buf_length / instreams), sizeof(char)*(buf_length / instreams)*2, cudaMemcpyDeviceToHost, streams[index*instreams + i]);
